...
if loop_check.Value == 1
    ...
    %26tasks‚Æ‚¢‚¤ƒfƒBƒŒƒNƒgƒŠ‚ª‚È‚¯‚ê‚Îì¬‚·‚é
    if exist('26tasks', 'file') == 0
        mkdir 26tasks;
    end
    
    %input_loop(•W€‚Í26‰ñ)‚Ü‚í‚µ‚Ädata.csv‚ğŠeü”g”‚²‚Æ‚Ìcsv‚É‚í‚¯‚é
    for i = 1:str2num(input_loop.String)
        ...
        %üŠú‚²‚Æ‚Éhorzcat‚·‚é Adjustment_Data‚ÌÅ‰‚Ì—ñ‚É‚Íƒ^[ƒQƒbƒg‚ª
        for j = 1:period
        ...
        min = zeros(0,0);
        max = zeros(0,0);
        %‘ƒf[ƒ^”‰ñ”‚Ü‚í‚· Split_AllData(k,3)‚ÉŠeâ‘ÎŒë·
...
%Šeƒ^ƒXƒN‚Ìƒf[ƒ^‚ğüŠú‚²‚Æ‚É‚Ü‚Æ‚ß‚é
